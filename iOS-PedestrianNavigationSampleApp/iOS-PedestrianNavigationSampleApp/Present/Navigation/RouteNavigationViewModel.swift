...
//                self?.getCurrentIndex(from: $0)
                self?.searchCurrentIndexByBinarySearch(from: $0, start: 0, end: (self?.navigationModels.count ?? 0) - 1)
    ...
    private func searchCurrentIndexByBinarySearch(from location: CLLocationCoordinate2D, start: Int, end: Int) {
        if start == end {
            ...
            let leftSideFrom = navigationModels[start].pointCoordinate
            let leftSideTo = navigationModels[end / 2].pointCoordinate
            ...
            let rightSideFrom = navigationModels[end / 2].pointCoordinate
            let rightSideTo = navigationModels[end].pointCoordinate
